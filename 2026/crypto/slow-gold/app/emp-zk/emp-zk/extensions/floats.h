...
inline Integer FloatToInt62(Float input, int s) {
  ...
  fraction =
      If(negexp >= Integer(8, 61, PUBLIC), Integer(61, 0, PUBLIC), fraction);
...
inline Float Int62ToFloat(Integer input, int s) {
  input = If(input > Integer(62, (1ULL << 60) - 1, PUBLIC),
             input - Integer(62, (1ULL << 61) - 1, PUBLIC), input);
  input.bits.pop_back();
  ...
  //	for(int i = 0; i < 61; ++i)cout << unsignedInput[i].reveal(PUBLIC);cout
  //<<endl;

  Integer firstOneIdx =
      Integer(8, 60, PUBLIC) - unsignedInput.leading_zeros().resize(8);
  Bit leftShift = firstOneIdx >= twentyThree;
  Integer shiftOffset =
      If(leftShift, firstOneIdx - twentyThree, twentyThree - firstOneIdx);
  Integer shifted =
      If(leftShift, unsignedInput >> shiftOffset, unsignedInput << shiftOffset);

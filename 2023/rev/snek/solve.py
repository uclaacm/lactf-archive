...
def dfs(loc, face, dest, avoid, tail):
    if loc == dest:
        return ("", face, deque(tail))
    dl = (-face[1], face[0])
    dr = (face[1], -face[0])
    pts = [
        (d, f, p)
        for (d, f, p) in [("1", face, add(loc, face)), ("L 1", dl, add(loc, dl)), ("R 1", dr, add(loc, dr))]
        if 0 <= p[0] < dim
        and 0 <= p[1] < dim
        and (p == dest or p not in avoid)
        and p not in tail
    ]
    pts.sort(key=lambda v: dist(v[2], dest))
    avoid.add(loc)
    for (d, f, p) in pts:
        istail = len(tail) != 0
        if istail:
            elem = tail.popleft()
            tail.append(loc)
        r = dfs(p, f, dest, avoid, tail)
        if istail:
            tail.pop()
            tail.appendleft(elem)
        if r is not None:
            return (d + " " + r[0], r[1], r[2])
    avoid.remove(loc)
    return None
